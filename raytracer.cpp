...
void RayTracer::render_tile(const Tile& tile, std::vector<Color>& out_pixels) {
    ...
    std::mt19937 rng(tile.id % tile.w + 12345); // Seed RNG for different snowflakes per rank
    const int snowflake_count = 75000; 
    std::vector<Vec3> snowflakes(snowflake_count);
    ...
    auto render_pixel = [&](int y, int x) {
            ...
            out_pixels[(y - tile.y0) * tile.w + (x - tile.x0)] = pixel_color;
    };
